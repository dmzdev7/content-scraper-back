...
export const sendVerificationEmail = async (
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to,
      subject: `Verifica tu email - ${config.app_name}`,
      html: `
        <!DOCTYPE html>
        <html lang="es">
        <head>
                  ...
                  <tr>
                    <td style="padding: 30px 40px; text-align: center; border-top: 1px solid #e2e8f0;">
                      ...
                      <p style="margin: 0; color: #a0aec0; font-size: 13px;">
                        © ${new Date().getFullYear()} ${config.app_name}. Todos los derechos reservados.
                      </p>
                    </td>
                  </tr>
                </table>
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                  ...
                  <tr>
                    <td style="padding: 0 40px 30px 40px;">
                      ...
                      <p style="margin: 0 0 16px 0; color: #4a5568; font-size: 16px; line-height: 1.6;">
                        Tu dirección de email ha sido verificada exitosamente. Ya puedes disfrutar de todas las funcionalidades de ${config.app_name}.
                      </p>
...
export const sendPasswordResetEmail = async (
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: config.email.from,
      to,
      subject: `Resetea tu contraseña - ${config.app_name}`,
      html: `
        <!DOCTYPE html>
        <html lang="es">
        <head>
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                  ...
                  <tr>
                    <td style="padding: 0 40px 20px 40px;">
                      ...
                      <p style="margin: 0 0 16px 0; color: #4a5568; font-size: 16px; line-height: 1.6;">
                        Recibimos una solicitud para resetear la contraseña de tu cuenta en ${config.app_name}.
                      </p>
                      <p style="margin: 0 0 24px 0; color: #4a5568; font-size: 16px; line-height: 1.6;">
...
export const sendPasswordChangedEmail = async (to: string, name: string) => {
  try {
    const { data, error } = await resend.emails.send({
      from: config.email.from,
      to,
      subject: `Contraseña cambiada - ${config.app_name}`,
      html: `
        <!DOCTYPE html>
        <html lang="es">
        <head>
